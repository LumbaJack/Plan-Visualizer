...
class StockGraphFunctions():
    ...
    def TR(self, date, close, high, low, open, yestclose):
        x = high - low
    ...
    def DM(self, date, open , high, low, close, yestopen, yesthigh, \
                                                            yestlow, yestclose):
    ...
    def calcDIs(self, date, closep, highp, lowp, openp):
        ...
        while x < len(date):
            ...
            DMdate, PosDM, NegDM = self.DM(date[x], openp[x], highp[x], \
                                           lowp[x], closep[x], openp[x-1], \
                                           highp[x-1], lowp[x-1], closep[x-1])
    ...
    def check_for_ma_alerts(self, date, ma1, ma2, plotEnt):
...
#                                                             data[-SP:], plotEnt)
    ...
    def check_for_trend_alerts(self, date, entry1, entry2, plotEnt):
        global SENDNOTIFICATION
...
#         self.check_for_mean_alerts(dates[-SP:], line[-SP:], data[-SP:], plotEnt)
    ...
    def check_for_mean_alerts(self, date, entry1, entry2, plotEnt):
        global SENDNOTIFICATION
        idx = np.argwhere(np.isclose(entry1, entry2, atol=.1)).reshape(-1)
    ...
    def graphData(self, MA1, MA2, timeframe=None, interval=None, symbol=None, \
                                        live=False, bot=False, botData=None):
        ''' Use this to dynamically pull a stock '''
        stockFile = list()

        initInput = self.get_http_response(interval, symbol)
...
#                                             len(date[-SP:]) == len(Av2[-SP:]):
#                 self.check_for_ma_alerts(date[-SP:], Av1[-SP:], Av2[-SP:], ax1)
              
            if live or bot:
                if not bot:
                    try:
                        self.segtrends(closep[-SP:], date[-SP:], ax1, SP, \
                                                                    segments=2)
                    except Exception, excp:
                        sys.stderr.write("ERROR: segtrends failed with " \
                                                        "exception %s\n" % excp)
                        pass
            ...
            #Uncomment this section for MACD            
...
#             self.ADX(date, closep, highp, lowp, openp, ax2, SP, plt)
            ...
            """ ax1.annotate('Buy Now!',(date[200],Av1[200]),
                xytext=(0.8, 0.9), textcoords='axes fraction',
                arrowprops=dict(facecolor='white', shrink=0.05),
                fontsize=14, color = 'w',
                horizontalalignment='right', verticalalignment='bottom') """
            ...
            if botData:
                for item in botData:
                    ax1.plot(item["ixd"],item["iyd"], color='w')
            ...
            if not live:
                ...
                mydir = os.path.join(os.getcwd(), 'live_trading_captures', \
                         symbol, botData[0]["name"], datetime.datetime.now().\
                         strftime('%Y-%m-%d_%H-%M-%S'))
                os.makedirs(mydir)
          
                fig.savefig(os.path.join(mydir, interval + ".png"), \
            ...
            if not bot:
                ...
                return os.path.join(mydir, interval + ".png")   
        except Exception, excp:
